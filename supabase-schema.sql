...
grant execute on function public.create_purchase(uuid,text,numeric,numeric,numeric,numeric,numeric,jsonb) to anon, authenticated;
grant execute on function public.create_sale(uuid,numeric,numeric,numeric,numeric,numeric,numeric,numeric,numeric,jsonb) to anon, authenticated;
...
-- TEMPORARY NO-AUTH MODE
-- The frontend uses a local SHA-256 password gate instead of Supabase Auth.
-- These policies allow the anon/publishable-key client to use the shop database.
-- This is intentionally temporary and should be replaced with proper server authentication before production.
do $$ begin
  create policy anon_profiles_select on public.profiles for select to anon using (true);
  create policy anon_settings_all on public.shop_settings for all to anon using (true) with check (true);
  create policy anon_categories_all on public.categories for all to anon using (true) with check (true);
  create policy anon_brands_all on public.brands for all to anon using (true) with check (true);
  create policy anon_units_select on public.units for select to anon using (true);
  create policy anon_products_all on public.products for all to anon using (true) with check (true);
  create policy anon_prices_all on public.price_history for all to anon using (true) with check (true);
  create policy anon_suppliers_all on public.suppliers for all to anon using (true) with check (true);
  create policy anon_customers_all on public.customers for all to anon using (true) with check (true);
  create policy anon_purchases_all on public.purchases for all to anon using (true) with check (true);
  create policy anon_purchase_items_all on public.purchase_items for all to anon using (true) with check (true);
  create policy anon_sales_all on public.sales for all to anon using (true) with check (true);
  create policy anon_sale_items_all on public.sale_items for all to anon using (true) with check (true);
  create policy anon_stock_all on public.stock_movements for all to anon using (true) with check (true);
  create policy anon_payments_all on public.payments for all to anon using (true) with check (true);
  create policy anon_expenses_all on public.expenses for all to anon using (true) with check (true);
  create policy anon_audit_all on public.audit_logs for all to anon using (true) with check (true);
exception when duplicate_object then null; end $$;

do $$ begin
  create policy invoice_storage_insert_anon on storage.objects for insert to anon with check(bucket_id='invoices');
  create policy invoice_storage_select_anon on storage.objects for select to anon using(bucket_id='invoices');
exception when duplicate_object then null; end $$;
